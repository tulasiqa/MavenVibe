...
public class MonatRL2Methods extends TestBase{
	...
	public static String vibeRecipient_text =  getEmail(adminUser_text,appUrl);
	  ...
	  public void navigate2UserRL(){
		...
		public void selectRankDefs() throws Exception{
			...
			int rankList = monatrankdeflist.length;
			...
			for(int i=1;i<=cntRanks;i++){
				...
				if(rankName.equalsIgnoreCase(monatrankdeflist[j]) ){  // && isSelected.equalsIgnoreCase("false")
					einput.click();
		...
		public void selectSubscriptionPlans() throws Exception{
			...
			for(int i=1;i<=cntRanks;i++){
				...
				// String isSelected = einput.getAttribute("checked").trim();
				//System.out.println("status=" +status);
				if(rankName.equalsIgnoreCase(monatsubplanslist[j]) ){  // && isSelected.equalsIgnoreCase("false")
					einput.click();
			     	break;
				}
			  }
...
//						waitOnElement("xpath",viewRes);
		...
		public void editProductCategory(String pcategory) throws Exception{
			logInfo("inside editProductCategory() method.");
	...
	// ============================================================================   
		...
		public boolean verifyResource(String resourceName) throws Exception, Exception{
			...
			boolean isResourcePresnt = false;			
			List <WebElement> cc = driver().findElements(By.cssSelector(resourceItems));  //resourceItems   resourceItemsHeader
			System.out.println("Resource "+ cc.size()); 
			
			String resBefore = "//*[@id='index_page']/div[1]/div/div[";
			String resAfter = "]/div/div[3]/div[1]";
			
			String resBefore1 = "//*[@id='index_page']/div[";
					String resAfter1 = "]/div/div";
			...
			for(int i=1;i<=cc.size();i++){
				String res = driver().findElement(By.xpath(resBefore1+i+resAfter1)).getText().trim();
				if(res.contains(resourceName)){
					isResourcePresnt=  true;
					WebElement e = driver().findElement(By.xpath(resBefore1+i+resAfter1));
					e.click();
					break;
				}
			}
	...
	public void previewResource(String resourceName) throws Exception{
...
/*	public void selectParentAndChildCategory(String parentCategory, String childCategory) throws Exception{
	...
	public void verifyLikeResources() throws Exception{		
		...
		for(int i=1 ; i<=ct.size(); i++){			
			...
			if (item.getText().equalsIgnoreCase("Likes")){
				WebElement countbefore = driver().findElement(By.xpath(monatlikesCount));
				...
				clickOnElement("xpath","//*[@id='page']/div[9]/div/div/div[2]/div[2]/div[2]/div[1]/section[3]/div/div[1]/a");
				waitOnElement("xpath","//*[@id='page']/div[9]/div/div/div[2]/div[2]/div[2]/div[1]/section[3]/div/div[1]/a");
				WebElement likes = driver().findElement(By.xpath("//*[@id='page']/div[9]/div/div/div[2]/div[2]/div[2]/div[1]/section[3]/div/div[1]/a"));			
				
				if(likes.getText().trim().contains("Liked")){
					WebElement likeCount = driver().findElement(By.xpath(monatlikesCount));
	...
	public void shareToCommunity() throws Exception{
		logInfo("Verify the social Networks section and select Share to Social sites."); 
		selectcontrols("Details");
		WebElement sc = driver().findElement(By.cssSelector(monatsharesCount)) ;
		...
		selectcontrols("Details");
		WebElement afterSharedcount = driver().findElement(By.cssSelector(monatsharesCount));	
