...
public class LpgnNewsMethods extends TestBase{
	...
	public void navigateCompanyNews() throws Exception{
		logInfo("inside navigateCompanyNews() method");
		driver().navigate().to(appUrl+"/pyr_core/company_news");
	}
	
	public void addCompanyNews(String newsTitle, boolean isNews, boolean isHighlights, boolean isPublishImmdly  ) throws Exception{
		logInfo("inside addCompanyNews() method");
		
		clickOnLink("linkText","Add Company News");
		inputTextClear("cssSelector", newsTitle);
		inputText("cssSelector", compTitle, newsTitle);
		inputText("cssSelector", compExcerpt, "new excerpt" );	// newsExcerpt
		composeText("xpath",composeBody, newsTitle  + "Company News description.");  //newsTitle
		rankDefinition();
		marketLanguages();
		selectSubscritionPlan();		
		uploadFile("Image",newsThumb);
		//uploadFile("PDF",newsPost);	
		selectIsNews(true);
		selectIsHighLight(true);
		selectPublishImmediately(true);
		clickOnButton("cssSelector", compPublish);	
		confirmationMessage("Company News is created");
	}
	
	
	public boolean verifyCompanyNewsIsPresent(String expTitle) throws Exception{
		logInfo("inside verifyCompanyNewsIsPresent() method.");
		
		navigateCompanyNews();
		
		boolean isNewsPresent = false;
		
		waitOnElement("cssSelector","#company_news");
		WebElement newsPane = driver().findElement(By.cssSelector("#company_news"));
		List allNews = newsPane.findElements(By.className("media-block"));
		System.out.println("allNews = " +allNews.size());
				
		String before_title = "#company_news > div:nth-child(";
		String after_title = ") > div.media-body > h4 > a";
		
		if(allNews.size() >0){
			for(int i=1;i<=allNews.size(); i++){
				WebElement etitle = driver().findElement(By.cssSelector(before_title+i+after_title));
				String actTitle = etitle.getText().trim();
				System.out.println("actTitle = " +actTitle);
				if(actTitle.equalsIgnoreCase(expTitle)){
					logInfo(expTitle + " Company news is present.");
					isNewsPresent = true;
					break;
				}
			}
		...
		if(isNewsPresent==false){
			logInfo(expTitle + " Company news is not present.");
			// Assert.assertTrue(isNewsPresent, expTitle + " Company news is not present.");
		}
	...
	public void deleteCompanyNews(String expTitle) throws Exception{
		logInfo("inside deleteCompanyNews() method.");
		...
		String before_delete = "#company_news > div:nth-child(";
		String after_delete = ") > div.media-actions > div:nth-child(1) > a:nth-child(2)";
		...
		if(allNews.size() >0){
			for(int i=1;i<=allNews.size(); i++){
				...
				if(actTitle.equalsIgnoreCase(expTitle)){
					logInfo(expTitle + " Company news is present.");
					isNewsPresent = true;
					WebElement delete = driver().findElement(By.cssSelector(before_delete+i+after_delete));
					delete.click();
					confirmOK();
					break;
				}
	...
	public void rankDefinition() throws Exception{ // String[] ranker
		logInfo("inside rankDefinition() method.");
		
		clickOnElement("cssSelector", compRank);		// expand Ranks
		
		waitOnElement("cssSelector",rankAll);
	...
	public void marketLanguages() throws Exception{  // String[] language
		logInfo("inside marketLanguages() method.");
	...
	public void selectSubscritionPlan() throws Exception{  // String[] SubcriptionPlan
			logInfo("inside selectSubscritionPlan() method.");
	...
	public void selectIsNews(boolean isNews) throws Exception, InterruptedException{
			logInfo("inside selectIsNews() method.");
		    		
			WebElement OV = driver().findElement(By.cssSelector(compIsNews));	
			if(isNews==true && !OV.isSelected()){
				OV.click();
			}
			
			if(isNews==false && OV.isSelected()){
				OV.click();
			}
		}
	...
	public void selectIsHighLight(boolean isHighlight) throws Exception, InterruptedException{
		logInfo("inside selectIsHighLight() method.");
		
			WebElement OV = driver().findElement(By.cssSelector(compIsHighLights));	
			
			if(isHighlight==true && !OV.isSelected()){
				OV.click();
			}
			
			if(isHighlight==false && OV.isSelected()){
				OV.click();
			}
	}
	
	public void selectPublishImmediately(boolean isPublishImmidly) throws Exception, InterruptedException{
		logInfo("inside selectPublishImmediately() method.");
		
		WebElement OV = driver().findElement(By.cssSelector(compPublishArticle));	
		
		if(isPublishImmidly==true && !OV.isSelected()){
			OV.click();
		}
		
		if(isPublishImmidly==false && OV.isSelected()){
			OV.click();
		}
	...
	public void isNewsTitlePresent(String title) throws Exception, InterruptedException{	
		Thread.sleep(5000);
		List<WebElement> newsList =  driver().findElements(By.cssSelector(companyNewsListTitle));
...
public void sortLanguages(String language){
    ...
    confirmationMessage(compNewsToasterText);
  	...
  	*/
  	...
  	loginAsUserFromAdvanced(firstName, lastName, consultantid);
