...
public class IDLRL2Methods extends TestBase{
	...
	public static String vibeRecipient_text =  getEmail(adminUser_text,appUrl);
	  ...
	  public void navigate2UserRL() throws IOException, Exception{
		...
		public void selectRankDefs() throws Exception{
			...
			int rankList = idlRankDeflist.length;
			...
			for(int i=1;i<=cntRanks;i++){
				...
				if(rankName.equalsIgnoreCase(idlRankDeflist[j]) ){  // && isSelected.equalsIgnoreCase("false")
					einput.click();
		...
		public void selectSubscriptionPlans() throws Exception{
			...
			int subList = idLifeSubPlansList.length;
			...
			for(int i=1;i<=cntRanks;i++){
				...
				// String isSelected = einput.getAttribute("checked").trim();
				//System.out.println("status=" +status);
				if(rankName.equalsIgnoreCase(idLifeSubPlansList[j]) ){  // && isSelected.equalsIgnoreCase("false")
					einput.click();
			     	break;
				}
			  }
...
//						waitOnElement("xpath",viewRes);
	...
	public boolean previewResource(String resourceName) throws Exception{
		...
		/*List<WebElement> rows = driver().findElements(By.xpath("//*[@id='index_page']/div/div"));
		  String before_res_row = "//*[@id='index_page']/div[1]/div[";
				int rowsCnt = rows.size();
		String res_row = "]/div/div[";
		String after_res_row = "]/div/div[2]/div";
		
		String beforeAction = "//*[@id='index_page']/div[";
		String resAction = "]/div/div[";
		String afterAction = "]/div[1]/div[1]/div";
		String preview = "/ul/li[1]";*/
		...
		/*for(int i=1; i<=rowsCnt; i++){
			...
			for(int j=1;j<=cols.size();j++){
				...
				WebElement res = driver().findElement(By.xpath(before_res_row+i+res_row+j+after_res_row));
				if (res.getText().equalsIgnoreCase(resourceName)){	
					 driver().findElement(By.xpath(beforeAction+i+resAction+j+afterAction)).click();
					 String previewResource = beforeAction+i+resAction+j+afterAction+preview;
					 waitOnElement("xpath", previewResource);
					 driver().findElement(By.xpath(previewResource)).click();
					 isResourceFound = true;
					 break;
				 }
			}
...
/*		 ( ( JavascriptExecutor ) _driver )
	...
	public boolean verifyParentCategory(String parentCategory) throws Exception{  
		 ...
		 String resourcePanelCategory = "//*[@class='panel-nav categories']/li";//"//*[@id='main-content']/div/div[2]/ul/li[@class!='active']";
		 String categoryName;
		 WebElement category;
		 ...
		 String before = "//*[@class='panel-nav categories']/li[";//"//*[@id='main-content']/div/div[2]/ul/li[@class!='active'][";
		 String after = "]/a";
		 for(int i=1;i<=categories.size();i++){
			 waitOnElement("xpath",before+i+after);
...
/*	public void selectParentAndChildCategory(String parentCategory, String childCategory) throws Exception{
	...
	public void verifyLikeResources() throws Exception{		
		...
		for(int i=1 ; i<=ct.size(); i++){			
			...
			if (item.getText().equalsIgnoreCase("Likes")){
				waitOnElement("xpath",idlLikesCount);
				WebElement countbefore = driver().findElement(By.xpath(idlLikesCount));
				...
				clickOnElement("xpath","//*[@id='page']/div[8]/div/div/div[2]/div[2]/div[2]/div[1]/section[3]/div/div[1]/a");
				waitOnElement("xpath","//*[@id='page']/div[8]/div/div/div[2]/div[2]/div[2]/div[1]/section[3]/div/div[1]/a");
				WebElement likes = driver().findElement(By.xpath("//*[@id='page']/div[8]/div/div/div[2]/div[2]/div[2]/div[1]/section[3]/div/div[1]/a"));			
				
				if(likes.getText().trim().contains("Liked")){
					waitOnElement("xpath",idlLikesCount);
					WebElement likeCount = driver().findElement(By.xpath(idlLikesCount));
					Assert.assertEquals(afterLikedCount, likeCount.getText().trim());		
				}
	...
	public boolean shareResourceByEmail(String resource) throws Exception, Exception{
		logInfo("Select Resource control like -Details, Share, Email  to foucs on it.");
		System.out.println("Inside shareResourceByEmail() method.");
		boolean isMailSent = false;
		waitOnElement("xpath",btnResEmail);
		clickOnElement("xpath",btnResEmail);
		//Still this workspace needs to be implemented for email
		return isMailSent;
	}
...
/*	public boolean shareToMyWebsite(String assetTitle) throws Exception{
		...
		waitOnElement("cssSelector",iconShare2Pwp);
		clickOnElement("cssSelector",iconShare2Pwp);
