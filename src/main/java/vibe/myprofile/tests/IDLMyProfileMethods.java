...
public class IDLMyProfileMethods extends TestBase {
	...
	public void verifyMyRecentActivity(String activity){
		...
		for (WebElement acts : act){
			...
			logInfo(activity + " status post match not found in myProfile.");
			Assert.assertTrue(isActivityPresent, activity + " status post match not found in  myProfile.");
		}		
  ...
  public void getPostsFromFBFrom2ndPart(String photoPost) throws Exception{    	  
	...
	clickOnElement("xpath", fbLogout); 
     ...
     public void verifyPostsInTwitter(String twitterPost) throws Exception{
       ...
       public IDLMyProfileMethods clickVedioLink() throws Exception{
...
//  		composeTextOnElement();
  		...
  		waitOnElement("cssSelector",".btn.btn-success.pull-right");
  		clickOnElement("cssSelector",".btn.btn-success.pull-right");
  	}
