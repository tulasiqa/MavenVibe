...
public class AVInboxMethods extends TestBase{
...
//				Thread.sleep(5000);
	...
	// Verify email deleted is displayed in the trashed folder.
		...
		public void composeEmailWithAttachment(String recipients, String subject) throws Exception {
			...
			addAttachment("Image");
			waitOnElement("cssSelector", send_Cal);
	...
	//		clickOnElement("cssSelector","form.simple_form.new_pyr_crm_message > input#message-attach-file-field");
			...
			clickOnButton("xpath", "//*[@id='upload_file_modal']/div/div[1]/div[3]/button[1]");
		...
		public boolean downloadAttachment(String fromRecepient) throws Exception, Exception{
		   ...
		   //Ganga
					public boolean openMailInGmail(String subject) throws Exception, Exception{ 
    ...
    public boolean gmailFromFieldREPID(String fieldName){ 	
    	   logInfo("inside gmailFromFieldREPID() method");
    	   boolean isEmailFieldVerified = false;
    	   WebElement emailName = driver().findElement(By.cssSelector(inboxField));
    	   logInfo("emailName = " +emailName.getText().trim());	
    	   if((emailName.isDisplayed())){
			logInfo(emailName.getText().trim() + " found..");
			Assert.assertEquals(emailName.getText().trim(),repEmailid);
    	   }else{
			logInfo(emailName.getText().trim() + " not found..");
			Assert.assertEquals(emailName.getText().trim(), repEmailid);
    	   }
    	   return isEmailFieldVerified;
    }
								
					
    public boolean gmailFromFieldDVM(String fieldName){ 	
        logInfo("inside gmailFromFieldDVM() method");
    	boolean isEmailFieldVerified = false;
    	WebElement emailName = driver().findElement(By.cssSelector(inboxField));
    	logInfo("emailName = " +emailName.getText().trim());
    	if((emailName.isDisplayed())){
    		logInfo(emailName.getText().trim() + " found..");
    		Assert.assertEquals(emailName.getText().trim(),avDvm);
    		}else{
    			logInfo(emailName.getText().trim() + " not found..");
    			Assert.assertEquals(emailName.getText().trim(), avDvm);
    		}
    	...
    	public boolean gmailFromFieldDSM(String fieldName){ 	
        logInfo("inside gmailFromFieldDSM() method");
    	boolean isEmailFieldVerified = false;
    	WebElement emailName = driver().findElement(By.cssSelector(inboxField));
    	logInfo("emailName = " +emailName.getText().trim());
    	if((emailName.isDisplayed())){
    	logInfo(emailName.getText().trim() + " found..");
    	Assert.assertEquals(emailName.getText().trim(),avDsm);
    	}else{
    		logInfo(emailName.getText().trim() + " not found..");
    		Assert.assertEquals(emailName.getText().trim(), avDsm);
    		}
    ...
    public void gmailReplyEmailVerificationForDVM(String subject) throws Exception{ 	
    	logInfo("inside gmailReplyEmailVerificationForDVM() method");
    	boolean isreplyEmailVerified = false;
    	loginGmail(gmailId_text2, gmailPwd_text1);
    	openMailInGmail(subject); //txtVibeEmailSubject
    	waitOnElement("cssSelector", "table[role=presentation] > tbody > tr > td:nth-child(2) > div > div > span:nth-child(1)");
    	clickOnElement("cssSelector", "table[role=presentation] > tbody > tr > td:nth-child(2) > div > div > span:nth-child(1)");
    	WebElement emailID = driver().findElement(By.cssSelector("form[method=POST] > div:nth-child(2) > table > tbody > tr > td:nth-child(2) > div:nth-child(2) > div:nth-child(1) > span"));
    	System.out.println(emailID.getText()+"is the unmasked email adress");
    	Assert.assertEquals(emailID.getText().trim(), dvmunMaskedEmail);
    	signoutGmail();
    ...
    public void gmailReplyEmailVerificationForDSM(String subject) throws Exception{ 	
    	boolean isreplyEmailVerified = false;
    	logInfo("inside gmailReplyEmailVerificationForDSM() method");
    	loginGmail(gmailId_text2, gmailPwd_text1);
    	openMailInGmail(subject); //txtVibeEmailSubject
    	waitOnElement("cssSelector", "table[role=presentation] > tbody > tr > td:nth-child(2) > div > div > span:nth-child(1)");
    	clickOnElement("cssSelector", "table[role=presentation] > tbody > tr > td:nth-child(2) > div > div > span:nth-child(1)");
    	WebElement emailID = driver().findElement(By.cssSelector("form[method=POST] > div:nth-child(2) > table > tbody > tr > td:nth-child(2) > div:nth-child(2) > div:nth-child(1) > span"));
    	System.out.println(emailID.getText()+"is the unmasked email adress");
    	Assert.assertEquals(emailID.getText().trim(), dsmunMaskedEmail);
    	signoutGmail();
    	logIn(adminUser_text, adminPwd_text);
    	loginAsUserFromAdvanced(prop.getLocatorForEnvironment(appUrl,"newsFN1"),prop.getLocatorForEnvironment(appUrl,"newsLN1"),
                prop.getLocatorForEnvironment(appUrl,"newsCon1"));
    ...
    public void gmailReplyEmailVerificationForREP(String subject) throws Exception{ 	
    	logInfo("inside gmailReplyEmailVerificationForREP() method");
    	boolean isreplyEmailVerified = false;
    	loginGmail(gmailId_text2, gmailPwd_text1);
    	openMailInGmail(subject); //txtVibeEmailSubject
    	waitOnElement("cssSelector", "table[role=presentation] > tbody > tr > td:nth-child(2) > div > div > span:nth-child(1)");
    	...
    	WebElement emailID = driver().findElement(By.cssSelector("form[method=POST] > div:nth-child(2) > table > tbody > tr > td:nth-child(2) > div:nth-child(2) > div:nth-child(1) > span"));
    	System.out.println(emailID.getText()+"is the unmasked email adress");
    	Assert.assertEquals(emailID.getText().trim(), repunMaskedEmail);
    	signoutGmail();
    	logIn(adminUser_text, adminPwd_text);
    	loginAsUserFromAdvanced(prop.getLocatorForEnvironment(appUrl,"newsFN1"),prop.getLocatorForEnvironment(appUrl,"newsLN1"),
                prop.getLocatorForEnvironment(appUrl,"newsCon1"));
