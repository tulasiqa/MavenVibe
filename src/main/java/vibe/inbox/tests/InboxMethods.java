...
public class InboxMethods extends TestBase {
	...
	public void loginGmail(String uname, String passwd) throws Exception {
		...
		/*
		 * Thread.sleep(3000); WebElement staysigned =
		 * driver().findElement(By.xpath(chkStaySignedIn)); String isChecked =
		 * staysigned.getAttribute("checked").trim();
		 * if(isChecked.equalsIgnoreCase("checked")){ staysigned.click(); }
		 */
		...
		/*
		 * WebElement profile =
		 * driver().findElement(By.xpath(drpdnGmailUserProfile));
		 * if(profile.isDisplayed()){ logInfo(profile.getText().trim() +
		 * " found.."); Assert.assertEquals(profile.getText().trim(),
		 * gmailId_text, "Gmail login sucessfull."); }else{
		 * logInfo(profile.getText().trim() + " not found..");
		 * Assert.assertEquals(profile.getText().trim(), gmailId_text,
		 * "Gmail login unsucessfull."); }
		 */
	}
	...
	public boolean verifyInboxGmail(String subject) {
		...
		if (isMatchFound == false) {
			...
			// Assert.assertTrue(isMatchFound, subject + " no email match
			// found");
		}
		return isMatchFound;
	}
	...
	public void signoutGmail() throws Exception {
		...
		if (profile.isDisplayed()) {
			...
			if (signIn.isDisplayed()) {
				...
				// String after_name =
				// "]/button[@type='submit']/span[@class='account-name']";
	...
	public void loginVibe(String userName, String passwd) throws Exception {
		 ...
		 * if(!UserName.isDisplayed()){ clickOnLink("xpath",lnkProfileDrpdn);
		 * clickOnLink("xpath",lnkLogout); Thread.sleep(5000); }
		 */
		...
		if (profile.isDisplayed()) {
			logInfo(profile.getText().trim() + " found..");
			// Assert.assertEquals(profile.getText().trim(), userName, " vibe
			// login sucessfull.");
		} else {
			logInfo(profile.getText().trim() + " not found..");
			// Assert.assertEquals(profile.getText().trim(), userName, " vibe
	...
	public void createLabel(String labelName) throws Exception {
		...
		Thread.sleep(3000);
		waitOnElement("linkText","Create New");
	...
	/*
	 * public void applyLabel(String labelName) throws Exception{ logInfo(
	 * "inside applyLabel() method..");
	 * verifyElementPresent("xpath",btnApplyLabel);
	 * clickOnButton("xpath",btnApplyLabel);
	 * 
	 * WebElement applyLabelPanels =
	 * driver().findElement(By.xpath(labelControl)); List<WebElement> li_menus =
	 * applyLabelPanels.findElements(By.tagName("li")); int li_count =
	 * li_menus.size();
	 * 
	 * String before = "//*[@id='bulk-controls']/div[2]/ul/li["; String after =
	 * "]/a";
	 * 
	 * for(int i=1; i<=li_count;i++){ WebElement li =
	 * driver().findElement(By.xpath(before+i+after)); String liName =
	 * li.getText().trim(); if(liName.equalsIgnoreCase(labelName)){ li.click();
	 * logInfo("clicked on menu label " + liName); break; } } //
	 * Thread.sleep(5000); }
	 */
	 ...
	 * 
	 * public void composeVibeEmail(String recipients, String subject) throws
	 * Exception { logInfo("inside composeVibeEmail() method..."); go2Inbox();
	 * 
	 * clickOnLink("linkText", "New Email");
	 * 
	 * WebElement composeTo =
	 * driver().findElement(By.cssSelector(recipientsTo)); // inputVibeComposeTo
	 * composeTo.click();
	 * 
	 ...
	 * 
	 * Robot rb = new Robot(); rb.delay(2000); rb.keyPress(KeyEvent.VK_TAB);
	 * rb.keyRelease(KeyEvent.VK_TAB); rb.delay(2000);
	 ...
	 * rb.keyPress(KeyEvent.VK_E); rb.keyRelease(KeyEvent.VK_E);
	 * rb.keyPress(KeyEvent.VK_TAB); rb.keyRelease(KeyEvent.VK_TAB);
	 * 
	 * verifyLinkPresent("Send"); clickOnLink("linkText","Send");
	 * 
	...
	// Compose an email with attachments and send to another (internal/external)
	// users.
	...
	public void composeVibeEmailwithAttachment(String recipients, String subject) throws Exception {
		...
		/*
		 * clickOnLink("linkText", "Attach a File"); Thread.sleep(5000);
		 * clickOnButton("xpath",btnBrowseFile);
		 * Runtime.getRuntime().exec(projectPath +
		 * "\\testdata\\auto-it\\fileUpload.exe"); Thread.sleep(15000);
		 * clickOnButton("xpath",attachToMessge); Thread.sleep(5000);
		 * clickOnButton("xpath",attachMailCLOSE); Thread.sleep(5000);
		 */
	...
	public boolean viewEmailsWithSubject(String subject) throws Exception {
		...
		/*
		 * String beforeSub = "//*[@id='bulk-form']/table/tbody/tr["; String
		 * afterSub = "]/td[2]";
		 */
		boolean isMatchFound = false;
		...
		for (int i = 1; i <= count - 1; i++) {
			...
			if (subject.contains(act_subject)) {
				isMatchFound = true;
				// WebElement sub =
				// driver().findElement(By.xpath(beforeSub+i+afterSub));
				// sub.click();
				x.click();
				logInfo("email with subject \'" + subject + "\' match found @" + i);
				Thread.sleep(5000);
	...
	public void submitReply(String subject) throws Exception, Exception {
		...
		/*
		 * JavascriptExecutor js = (JavascriptExecutor)driver; WebElement send =
		 * driver().findElement(By.linkText("Send"));
	...
	public boolean downloadAttachment(String fromRecepient) throws Exception, Exception {
	...
	public void addSignature(String sign) throws Exception {
		logInfo("inside addSignature method ");
		profile.navigateMyProfileAccount("Email Signatures");
		...
		inputText("cssSelector", "#pyr_crm_email_signature_name", sign);
		composeText("xpath", composeBody, composeBodySignatureText);
	...
	public boolean makeSignatureasDefault(String signName) throws Exception {
		logInfo("inside makeSignatureasDefault method ");
		List<WebElement> totalSigns = driver().findElements(By.xpath("//*[@id='email_signatures_tab']/div"));
		int signsSize = totalSigns.size();
		System.out.println(signsSize);
		boolean isSignPresent = false;
		for (int i = 2; i <= signsSize; i++) {
			String beforePath = "//*[@id='email_signatures_tab']/div[";
			String afterPath = "]/div/span/h5/strong";
			String dropDown = "]/div/div[1]";
			String markasDefault = "]/div/div[1]/ul/li[1]";
			...
			if (title.equalsIgnoreCase(signName)) {
				...
				WebElement option = driver().findElement(By.xpath(beforePath + i + markasDefault));
	...
	public boolean verifyAddedSignatureInNewEmail(String Signature) throws Exception {
		logInfo("inside verifyAddedSignatureInNewEmail method");
		  go2Inbox();
		  clickOnLink("linkText", "New Email");
		  ...
		  String expectedText =  body.getText().trim();
		  System.out.println(expectedText +"is the expected signature");
		  if(expectedText.equalsIgnoreCase(Signature)){
			    driver().switchTo().defaultContent();
			    isSignaturePresent =true;
			    System.out.println("erwerer");
				Assert.assertTrue(isSignaturePresent, expectedText + "is the  Singature  showing in the message body ");
		/*	    
			//    clickOnLink("linkText", "New Email");
			    waitOnElement("xpath",signLink);
			 clickOnElement("xpath",signLink);
			 List<WebElement>signaturesList = driver().findElements(By.xpath(manageSignatures));
			 int signlistSize = signaturesList.size();
			 System.out.println(signlistSize);
			 String path1  = "//*[@id='new_pyr_crm_message']/span[1]/div/div[1]/span[2]/div/ul/li[";
			 String path2 = "]/a";
			 for(int i=signlistSize;i>=0;i--){
					WebElement x = driver().findElement(By.xpath(path1+i+path2));
					String title = x.getText().trim();
					System.out.println("title is " + title);
					if (title.equalsIgnoreCase("Manage Signatures")) {
						x.click();				 
						break;
	...
	*/	  
	...
	public void  deleteDefaultSingature(String signName) throws Exception {
		logInfo("inside deleteDefaultSingature method ");
			waitOnElement("xpath",signLink);
			clickOnElement("xpath",signLink);
			List<WebElement>signaturesList = driver().findElements(By.xpath(manageSignatures));
			int signlistSize = signaturesList.size();
			...
			for(int i=signlistSize;i>=0;i--){
				WebElement x = driver().findElement(By.xpath(path1+i+path2));
				String signTitle = x.getText().trim();
				System.out.println("title is " + signTitle);
				if (signTitle.equalsIgnoreCase("Manage Signatures")) {
					x.click();				 
					break; 
					}
		List<WebElement> totalSigns = driver().findElements(By.xpath("//*[@id='email_signatures_tab']/div"));
		int signsSize = totalSigns.size();
		System.out.println(signsSize);
		for (int j = 2;j <= signsSize; j++) {
			String beforePath = "//*[@id='email_signatures_tab']/div[";
			String afterPath = "]/div/span/h5/strong";
			String dropDown = "]/div/div[1]";
			String markasDefault = "]/div/div[1]/ul/li[3]";
