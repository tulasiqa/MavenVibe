...
public class LPGNRL2Methods extends TestBase{
	...
	public static String vibeRecipient_text =  getEmail(adminUser_text,appUrl);
	  ...
	  public void navigate2UserRL(){
		...
		public void selectRankDefs() throws Exception{
			...
			int rankList = lpgnrankdeflist.length;
			...
			for(int i=1;i<=cntRanks;i++){
				...
				if(rankName.equalsIgnoreCase(lpgnrankdeflist[j]) ){  // && isSelected.equalsIgnoreCase("false")
					einput.click();
		...
		public void selectSubscriptionPlans() throws Exception{
			...
			int subList = lpgnSubPlansList.length;
			...
			for(int i=1;i<=cntRanks;i++){
				...
				// String isSelected = einput.getAttribute("checked").trim();
				//System.out.println("status=" +status);
				if(rankName.equalsIgnoreCase(lpgnSubPlansList[j]) ){  // && isSelected.equalsIgnoreCase("false")
					einput.click();
			     	break;
				}
			  }
	...
	public void verifyLikeResources() throws Exception{		
		...
		for(int i=1 ; i<=ct.size(); i++){			
			...
			if (item.getText().equalsIgnoreCase("Likes")){
				WebElement countbefore = driver().findElement(By.xpath(lpgnlikesCount));
				...
				if(likes.getText().trim().contains("Liked")){
					WebElement likeCount = driver().findElement(By.xpath(lpgnlikesCount));
